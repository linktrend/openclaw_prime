...
- Carlos may assign work directly to any agent on any platform, machine, or surface. No Orchestrator is a permission gate between Carlos and another agent. A directly assigned agent records Carlos's instruction, checks current coordination state, and proceeds within that scope; if ownership overlaps, pause only the conflicting action and report the collision to Carlos and the relevant Orchestrator.
