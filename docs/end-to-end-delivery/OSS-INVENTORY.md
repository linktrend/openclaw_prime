...
| Package manager | `pnpm@12.1.0` pinned by package manager hash | Global shim selects an unbuilt placeholder and returns `ENOEXEC` | Use the documented task-local exact-version install with build scripts; do not change the global shim or package manager. |
...
| Primary model | OpenAI provider + Codex app server | Configured Sol Low; Lisa auth refresh failed in a real turn | Govern source repair; the routine attempt is covered by task approval, while an actual interactive login/consent needs founder presence. |
...
| Coding delegation | Existing `sessions_spawn`, target policy, tool inheritance, ACP/ACPX surfaces | Existing delegation mode is prompt-only and ordinary deny policy is inherited into ACP | Add OCP-02's generic direct-coding wrapper and configured `development-orchestrator` target; no new agent framework. |
...
| Private network | Existing Server01/Tailscale/reverse-proxy foundation | Agent ports currently loopback-only | Preserve; expose only the authenticated Google Chat webhook in OCP-13 after core acceptance. |
