...
- **Pipeline Ship/Pull/Promote** — Procedure: `agents/pipeline-status.md`. Status file: `memory/pipeline-status.md`. One-line Telegram statuses only (`Ship A: Clear`, etc.). Main Approve ~Mon 08:30 via Telegram. Doctrine lives in IDE Development `docs/AUTONOMOUS-GIT-OPERATIONS.md`.
...
- **Pipeline Ship/Pull/Promote** — Procedure: `agents/pipeline-status.md`. Status: `memory/pipeline-status.md`. Include Telegram section D one-liners when known. Mon ~08:30 Main Approve via Telegram.
